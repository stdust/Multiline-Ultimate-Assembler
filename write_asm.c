...
LONG_PTR WriteAsm(TCHAR *lpText, TCHAR *lpError)
{
	...
	// Automatically find and allocate Code Caves!
	if (!lpErrorSpot)
	{
		CMD_BLOCK_NODE *cmd_block_node;
		for (cmd_block_node = cmd_block_head.next; cmd_block_node != NULL; cmd_block_node = cmd_block_node->next)
		{
			if (cmd_block_node->bIsCodeCave)
			{
				if (cmd_block_node->nSize == 0)
				{
					lstrcpy(lpError, _T("Empty code cave is not allowed"));
					lpErrorSpot = lpText;
					break;
				}
				if (!cmd_block_node->dwEndAddress)
				{
					lstrcpy(lpError, _T("The code cave block must follow a valid hook block"));
					lpErrorSpot = lpText;
					break;
				}
				
				DWORD_PTR dwCaveAddress = FindCodeCave(cmd_block_node->dwEndAddress, cmd_block_node->nSize, lpError);
				if (!dwCaveAddress)
				{
					lpErrorSpot = lpText;
					break;
				...
				cmd_block_node->dwAddress = dwCaveAddress;
				
				// Reserve the cave memory by writing INT3 (0xCC) placeholder bytes.
				// This prevents subsequent FindCodeCave calls from finding the
				// same zero-filled region and returning an overlapping address.
				// PatchCommands will overwrite these with the actual cave code later.
				{
					PLUGIN_MEMORY caveMem = FindMemory(dwCaveAddress);
					if (caveMem)
						EnsureMemoryBackup(caveMem);

					BYTE *pReserve = (BYTE *)HeapAlloc(GetProcessHeap(), 0, cmd_block_node->nSize);
					if (pReserve)
					{
						FillMemory(pReserve, cmd_block_node->nSize, 0xCC);
						SimpleWriteMemory(pReserve, dwCaveAddress, cmd_block_node->nSize);
						HeapFree(GetProcessHeap(), 0, pReserve);
					}
				}
				
				if (!AddCaveLabel(&label_head, cmd_block_node->szCaveName, dwCaveAddress, lpError))
				{
					lpErrorSpot = lpText;
					break;
				}
	...
	// Re-assemble commands in code cave blocks at the correct address.
	// During TextToData, code cave commands are assembled at the hook block's end
	// address (a temporary address). Now that FindCodeCave has determined the real
	// cave address, we must re-assemble non-label commands so that RIP-relative
	// encodings (used by x64 for call/jmp [addr]) point to the correct target.
	if (!lpErrorSpot)
		lpErrorSpot = ReassembleCaveCommands(&cmd_block_head, lpError);
...
{
	DWORD_PTR dwAddress;
	BOOL bIsCodeCave = FALSE;
	TCHAR szCaveName[64] = {0};

	DWORD_PTR dwPrevEndAddress = *pdwAddress;

	LONG_PTR result = ParseAddress(lpText, &dwAddress, pdwEndAddress, pdwBaseAddress, &bIsCodeCave, szCaveName, lpError);
	if(result <= 0)
		return result;

	if(!NewCmdBlock(p_cmd_block_head, dwAddress, bIsCodeCave, szCaveName, lpError))
		return 0;
	...
	*p_cmd_block_node = p_cmd_block_head->last;
	...
	if (bIsCodeCave)
	{
		(*p_cmd_block_node)->dwEndAddress = dwPrevEndAddress;
		*pdwAddress = dwPrevEndAddress;
		*pdwEndAddress = 0;
		*pdwBaseAddress = 0;
	}
	else
...
static LONG_PTR ParseAddress(TCHAR *lpText, DWORD_PTR *pdwAddress, DWORD_PTR *pdwEndAddress, DWORD_PTR *pdwBaseAddress, BOOL *pbIsCodeCave, TCHAR *pszCaveName, TCHAR *lpError)
{
	...
	// Check for codecave prefix: <codecave:name> or single name like <name>
	TCHAR *szPrefix = _T("codecave:");
	int nPrefixLen = lstrlen(szPrefix);
	BOOL bIsCavePrefix = (memcmp(p, szPrefix, nPrefixLen * sizeof(TCHAR)) == 0);

	TCHAR *pEnd = p;
	while (*pEnd != _T('>') && *pEnd != _T('\0'))
	{
		pEnd++;
	}
	...
	if (*pEnd == _T('>'))
	{
		...
		// If no '..' dots and not starting with '$' or numbers (implying a dynamic cave name), OR starts with 'codecave:'
		if (bIsCavePrefix || (!bHasDots && *p != _T('$') && (*p < _T('0') || *p > _T('9'))))
		{
			if (bIsCavePrefix)
			{
				p += nPrefixLen;
			...
			TCHAR *pStart = p;
			while (p < pEnd && *p != _T(' ') && *p != _T('\t'))
			{
				p++;
			}
			int nNameLen = (int)(p - pStart);
			if (nNameLen == 0 || nNameLen >= 64)
			{
				lstrcpy(lpError, _T("Invalid codecave name"));
				return -(pStart-lpText);
			}

			// Validate name characters (alphanumerics, underscore, at-sign)
			for (int i = 0; i < nNameLen; i++)
			{
				...
				if (!((c >= _T('A') && c <= _T('Z')) || 
				      ...
				      c == _T('_') || c == _T('@')))
				{
					lstrcpy(lpError, _T("Invalid characters in codecave name"));
					return -(pStart-lpText);
				}
			}

			*pbIsCodeCave = TRUE;
			lstrcpyn(pszCaveName, pStart, nNameLen + 1);

			p = SkipSpaces(p);
			if (p != pEnd)
			{
				lstrcpy(lpError, _T("Could not parse codecave, '>' expected"));
				return -(p-lpText);
			}
			p++; // skip '>'
			*pdwAddress = 0;
...
static BOOL NewCmdBlock(CMD_BLOCK_HEAD *p_cmd_block_head, DWORD_PTR dwAddress, BOOL bIsCodeCave, TCHAR *pszCaveName, TCHAR *lpError)
{
	...
	cmd_block_node->bIsCodeCave = bIsCodeCave;
	if (bIsCodeCave && pszCaveName) {
		lstrcpyn(cmd_block_node->szCaveName, pszCaveName, 64);
	} else {
		cmd_block_node->szCaveName[0] = _T('\0');
	}
	cmd_block_node->dwEndAddress = 0;
...
static LONG_PTR ParseLabel(TCHAR *lpText, DWORD_PTR dwAddress, LABEL_HEAD *p_label_head, DWORD_PTR *pdwPaddingSize, TCHAR *lpError)
{
	...
	*lpLabelEnd = _T('\0');
	...
	if(lstrcmp(lpLabel, _T("return")) == 0)
	{
		lstrcpy(lpError, _T("The label name 'return' is reserved"));
		return 0;
	}
		...
		for(cmd_node = cmd_block_node->cmd_head.next; cmd_node != NULL; cmd_node = cmd_node->next)
		{
			...
			if(cmd_node->lpResolvedCommandWithLabels)
			{
				result = ReplaceLabelsFromList(cmd_node->lpResolvedCommandWithLabels, dwPrevAnonAddr, dwNextAnonAddr, 
					p_label_head, p_cmd_block_head, cmd_block_node, &lpCommandWithoutLabels, lpError);
...
static LONG_PTR ReplaceLabelsFromList(TCHAR *lpCommand, DWORD_PTR dwPrevAnonAddr, DWORD_PTR dwNextAnonAddr,
	LABEL_HEAD *p_label_head, CMD_BLOCK_HEAD *p_cmd_block_head, CMD_BLOCK_NODE *cmd_block_node, TCHAR **ppNewCommand, TCHAR *lpError)
{
	LABEL_NODE *label_node;
	TCHAR *p;
	LONG_PTR text_start[4];
	...
	while(*p != _T('\0') && *p != _T(';'))
	{
		...
		{
			// Check for <codecave:name> or <name>
			TCHAR *pStart = p;
			TCHAR *pEnd = pStart;
			while(*pEnd != _T('\0') && *pEnd != _T('>') && *pEnd != _T(';'))
			{
				pEnd++;
			}
			...
			if(*pEnd == _T('>'))
			{
				...
				text_start[label_count] = pStart - lpCommand;
				text_end[label_count] = (pEnd + 1) - lpCommand; // Include '>' in substitution
	...
	// Find these labels/caves in our lists
	for(i=0; i<label_count; i++)
	{
		p = lpCommand+text_start[i]+1; // Skip the '@' or '<' char

		// If it's a code cave token (starts with '<')
		if (lpCommand[text_start[i]] == _T('<'))
		{
			temp_char = lpCommand[text_end[i] - 1];
			lpCommand[text_end[i] - 1] = _T('\0');
			
			TCHAR *pszCaveName = p;
			TCHAR *szPrefix = _T("codecave:");
			int nPrefixLen = lstrlen(szPrefix);
			if (memcmp(pszCaveName, szPrefix, nPrefixLen * sizeof(TCHAR)) == 0)
			{
				pszCaveName += nPrefixLen;
			}
			
			// Trim space padding if any
			while(*pszCaveName == _T(' ') || *pszCaveName == _T('\t'))
				pszCaveName++;
			int nLen = lstrlen(pszCaveName);
			while(nLen > 0 && (pszCaveName[nLen - 1] == _T(' ') || pszCaveName[nLen - 1] == _T('\t')))
			{
				pszCaveName[nLen - 1] = _T('\0');
				nLen--;
			}
			
			// Find this cave name in our code cave blocks
			CMD_BLOCK_NODE *curr_block;
			BOOL bFound = FALSE;
			for (curr_block = p_cmd_block_head->next; curr_block != NULL; curr_block = curr_block->next)
			{
				if (curr_block->bIsCodeCave && lstrcmpi(curr_block->szCaveName, pszCaveName) == 0)
				{
					dwAddress[i] = curr_block->dwAddress;
					bFound = TRUE;
					break;
				}
			}
			...
			// Restore character
			lpCommand[text_end[i] - 1] = temp_char;
			
			if (!bFound)
			{
				lstrcpy(lpError, _T("The code cave was not defined"));
				return -text_start[i];
			}
			continue;
		}
		...
		if(lstrcmp(p, _T("return")) == 0)
		{
			lpCommand[text_end[i]] = temp_char;
			if (!cmd_block_node->bIsCodeCave || !cmd_block_node->dwEndAddress)
			{
				lstrcpy(lpError, _T("The @return label can only be used inside a codecave that follows a hook block"));
				return -text_start[i];
			}
			dwAddress[i] = cmd_block_node->dwEndAddress;
			continue;
		}
...
// Re-assemble all non-label commands in code cave blocks at their real addresses.
// This fixes x64 RIP-relative addressing: during TextToData, cave commands are
// assembled at a provisional address (the hook block end). After FindCodeCave
// assigns the real address, we must re-assemble so that instructions like
// "call qword ptr [0x1400640F0]" encode the correct RIP-relative offset.
static TCHAR *ReassembleCaveCommands(CMD_BLOCK_HEAD *p_cmd_block_head, TCHAR *lpError)
{
	CMD_BLOCK_NODE *cmd_block_node;
	CMD_NODE *cmd_node;
	DWORD_PTR dwAddress;
	...
	for (cmd_block_node = p_cmd_block_head->next; cmd_block_node != NULL; cmd_block_node = cmd_block_node->next)
	{
		if (!cmd_block_node->bIsCodeCave || cmd_block_node->dwAddress == 0)
			continue;
		...
		for (cmd_node = cmd_block_node->cmd_head.next; cmd_node != NULL; cmd_node = cmd_node->next)
		{
			// Only re-assemble commands that have no labels.
			// Commands with labels (lpResolvedCommandWithLabels != NULL) will be
			// re-assembled later in ReplaceLabelsInCommands with the correct address.
			if (cmd_node->lpCommand && cmd_node->nCodeSize > 0 && !cmd_node->lpResolvedCommandWithLabels)
			{
				nNewSize = AssembleShortest(cmd_node->lpCommand, dwAddress, bCode, lpError);
				if (nNewSize > 0 && (SIZE_T)nNewSize == cmd_node->nCodeSize)
				{
					CopyMemory(cmd_node->bCode, bCode, nNewSize);
				}
				else if (nNewSize > 0 && (SIZE_T)nNewSize != cmd_node->nCodeSize)
				{
					// Size changed - reallocate buffer if new size is larger
					if ((SIZE_T)nNewSize > cmd_node->nCodeSize)
					{
						BYTE *pNewBuf = (BYTE *)HeapAlloc(GetProcessHeap(), 0, nNewSize);
						if (!pNewBuf)
						{
							lstrcpy(lpError, _T("Allocation failed during cave re-assembly"));
							return cmd_node->lpCommand;
						}
						HeapFree(GetProcessHeap(), 0, cmd_node->bCode);
						cmd_node->bCode = pNewBuf;
					}
					CopyMemory(cmd_node->bCode, bCode, nNewSize);

					// Adjust block total size
					cmd_block_node->nSize = cmd_block_node->nSize - cmd_node->nCodeSize + nNewSize;
					cmd_node->nCodeSize = nNewSize;
				}
				// If assembly fails (nNewSize <= 0), keep the original bytes.
				// This handles non-instruction data nodes (strings, byte inserts)
				// whose lpCommand text wouldn't assemble as an instruction.
			}
...
// Find the PE module base address by scanning backward from the given address
// looking for the MZ/PE signature. Module bases are always page-aligned.
static DWORD_PTR FindModuleBase(DWORD_PTR dwAddress)
{
	IMAGE_DOS_HEADER dosHeader;
	DWORD peSignature;
	DWORD_PTR dwBase = dwAddress & ~(DWORD_PTR)0xFFF; // Align to page boundary
	int nMaxPages = 0x10000; // Search up to ~256MB backward
	int i;

	for (i = 0; i < nMaxPages; i++)
	{
		if (SimpleReadMemory(&dosHeader, dwBase, sizeof(dosHeader)))
		{
			if (dosHeader.e_magic == IMAGE_DOS_SIGNATURE)
			{
				...
				{
					peSignature = 0;
					if (SimpleReadMemory(&peSignature, dwBase + dosHeader.e_lfanew, sizeof(peSignature)))
					{
...
// Get the SizeOfRawData for the PE section containing dwAddress.
// This is the file-backed portion of the section. Bytes beyond this
// exist in memory (zero-filled) but NOT in the PE file on disk.
// Returns 0 if the section cannot be determined.
static SIZE_T GetSectionRawSize(DWORD_PTR dwAddress)
{
	IMAGE_DOS_HEADER dosHeader;
	...
	DWORD_PTR dwPEAddr;
	DWORD_PTR dwSectionTable;
	WORD i;

	dwModuleBase = FindModuleBase(dwAddress);
	if (!dwModuleBase)
		return 0;

	// Read DOS header
	if (!SimpleReadMemory(&dosHeader, dwModuleBase, sizeof(dosHeader)))
		return 0;
	if (dosHeader.e_magic != IMAGE_DOS_SIGNATURE)
		return 0;

	// Read PE signature
	dwPEAddr = dwModuleBase + dosHeader.e_lfanew;
	peSignature = 0;
	if (!SimpleReadMemory(&peSignature, dwPEAddr, sizeof(peSignature)))
		return 0;
	if (peSignature != IMAGE_NT_SIGNATURE)
		return 0;

	// Read COFF file header (immediately after PE signature)
	if (!SimpleReadMemory(&fileHeader, dwPEAddr + 4, sizeof(fileHeader)))
		return 0;

	// Section table starts after optional header
	dwSectionTable = dwPEAddr + 4 + sizeof(IMAGE_FILE_HEADER) + fileHeader.SizeOfOptionalHeader;

	// Find the section containing our address
	for (i = 0; i < fileHeader.NumberOfSections; i++)
	{
		if (!SimpleReadMemory(&sectionHeader, dwSectionTable + i * sizeof(IMAGE_SECTION_HEADER), sizeof(sectionHeader)))
			continue;

		DWORD_PTR dwSecStart = dwModuleBase + sectionHeader.VirtualAddress;
		DWORD_PTR dwSecEnd = dwSecStart + sectionHeader.Misc.VirtualSize;

		if (dwAddress >= dwSecStart && dwAddress < dwSecEnd)
		{
			return (SIZE_T)sectionHeader.SizeOfRawData;
		}
	}
...
static DWORD_PTR FindCodeCave(DWORD_PTR dwHookAddress, SIZE_T nRequiredSize, TCHAR *lpError)
{
	PLUGIN_MEMORY memory = FindMemory(dwHookAddress);
	if(!memory)
	{
		lstrcpy(lpError, _T("Failed to find memory block for hook address"));
		return 0;
	}

	DWORD_PTR dwMemoryBase = GetMemoryBase(memory);
	SIZE_T nMemorySize = GetMemorySize(memory);

	// Limit scan to the file-backed raw data area of the PE section.
	// Beyond SizeOfRawData, memory is zero-filled but not in the PE file,
	// so caves there can't be saved by Scylla dump or x64dbg patch-to-file.
	{
		SIZE_T nRawSize = GetSectionRawSize(dwHookAddress);
		if (nRawSize > 0 && nRawSize < nMemorySize)
			nMemorySize = nRawSize;
	}

	return ScanMemoryForCave(dwMemoryBase, nMemorySize, nRequiredSize, lpError);
}

static DWORD_PTR ScanMemoryForCave(DWORD_PTR dwBase, SIZE_T nSize, SIZE_T nRequiredSize, TCHAR *lpError)
{
	SIZE_T nScanSize = nRequiredSize + 8; // Leave 4 bytes of padding before and after
	BYTE *bChunk = (BYTE *)HeapAlloc(GetProcessHeap(), 0, 4096);
	if (!bChunk)
	{
		lstrcpy(lpError, _T("Allocation failed during cave scanning"));
		return 0;
	}

	SIZE_T nConsecutive = 0;
	DWORD_PTR dwCaveStart = 0;
	SIZE_T nChunkSize = 4096;

	for (DWORD_PTR dwAddr = dwBase; dwAddr < dwBase + nSize; )
	{
		SIZE_T nBytesToRead = nChunkSize;
		if (dwAddr + nBytesToRead > dwBase + nSize)
		{
			nBytesToRead = (dwBase + nSize) - dwAddr;
		}

		if (nBytesToRead == 0)
			break;

		if (!SimpleReadMemory(bChunk, dwAddr, nBytesToRead))
		{
			dwAddr += nBytesToRead;
			nConsecutive = 0;
			continue;
		...
		for (SIZE_T i = 0; i < nBytesToRead; i++)
		{
			...
			if (b == 0x00 || b == 0x90)
			{
				if (nConsecutive == 0)
				{
					dwCaveStart = dwAddr + i;
				}
				nConsecutive++;
				if (nConsecutive >= nScanSize)
				{
					HeapFree(GetProcessHeap(), 0, bChunk);
					return dwCaveStart + 4; // Return address with 4-byte prefix padding
				}
			}
			else
			{
	...
	HeapFree(GetProcessHeap(), 0, bChunk);
	lstrcpy(lpError, _T("No suitable code cave found of the required size"));
	return 0;
}

static BOOL AddCaveLabel(LABEL_HEAD *p_label_head, TCHAR *lpLabel, DWORD_PTR dwAddress, TCHAR *lpError)
{
	LABEL_NODE *label_node = (LABEL_NODE *)HeapAlloc(GetProcessHeap(), 0, sizeof(LABEL_NODE));
	if (!label_node)
	{
		lstrcpy(lpError, _T("Allocation failed"));
		return FALSE;
