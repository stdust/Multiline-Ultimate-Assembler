...
// Builds "<directory of hInst>\<pFileName>" in pszResult (MAX_PATH TCHARs).
// Returns FALSE if the module path cannot be retrieved or the result would
// not fit, instead of silently truncating or overflowing.
static BOOL BuildPathNextToModule(HINSTANCE hInst, const TCHAR *pFileName, TCHAR *pszResult)
{
	DWORD dwLen;
	int nDirLen, nFileLen;
	TCHAR *p;

	dwLen = GetModuleFileName(hInst, pszResult, MAX_PATH);
	if(dwLen == 0 || dwLen >= MAX_PATH)
	{
		// dwLen == MAX_PATH means the buffer was too small and, on older
		// systems, the result is not NUL terminated.
		pszResult[0] = _T('\0');
		return FALSE;
	}

	p = _tcsrchr(pszResult, _T('\\'));
	if(p)
		nDirLen = (int)(p - pszResult) + 1;
	else
		nDirLen = 0;

	nFileLen = lstrlen(pFileName);
	...
	lstrcpy(pszResult + nDirLen, pFileName);
	return TRUE;
}
...
static void EnsureIniFileExists(HINSTANCE hInst)
{
	...
	if(!BuildPathNextToModule(hInst, _T("multiasm.ini"), szIniPath))
		return;

	dwAttrib = GetFileAttributes(szIniPath);
...

	{
		static const char defaultIniData[] =
			...
			"\r\n"
			"; --- JMP (점프) ---\r\n"
			"color_jmp          = 0xFF0000   ; jmp/je/js 키워드 글자색\r\n"
			...
			"color_reg_special  = 0x800000   ; 세그먼트 레지스터 글자색\r\n"
		...
		DWORD dwWritten = 0;
		WriteFile(hFile, defaultIniData, (DWORD)(sizeof(defaultIniData) - 1), &dwWritten, NULL);
	}
...
TCHAR *PluginInit(HINSTANCE hInst)
{
	...
	// Load Scintilla.dll
	// 1. Try loading from the plugin's own directory first. Loading by full
	//    path avoids picking up an unrelated Scintilla.dll from the search
	//    path.
	if(BuildPathNextToModule(hInst, _T("Scintilla.dll"), szDllPath))
		hScintillaDLL = LoadLibrary(szDllPath);

	// 2. Fallback to the standard search path if that failed
	if (!hScintillaDLL)
	...
	// Load options.
	//
	// NOTE: MyGetintfromini() validates the [min, max] range only when
	// min != max. Boolean options therefore pass (0, 1) so that a corrupted
	// ini value cannot smuggle an arbitrary integer into a flag.
	MyGetintfromini(hInst, _T("disasm_rva"), &options.disasm_rva, 0, 1, 1);
	MyGetintfromini(hInst, _T("disasm_rva_reloconly"), &options.disasm_rva_reloconly, 0, 1, 1);
	MyGetintfromini(hInst, _T("disasm_label"), &options.disasm_label, 0, 1, 1);
	MyGetintfromini(hInst, _T("disasm_extjmp"), &options.disasm_extjmp, 0, 1, 1);
	MyGetintfromini(hInst, _T("disasm_hex"), &options.disasm_hex, 0, OPT_DISASM_HEX_MAX, 0);
	MyGetintfromini(hInst, _T("disasm_labelgen"), &options.disasm_labelgen, 0, OPT_DISASM_LABELGEN_MAX, 0);
	MyGetintfromini(hInst, _T("asm_comments"), &options.asm_comments, 0, 1, 1);
	MyGetintfromini(hInst, _T("asm_labels"), &options.asm_labels, 0, 1, 1);
	MyGetintfromini(hInst, _T("edit_savepos"), &options.edit_savepos, 0, 1, 1);
	MyGetintfromini(hInst, _T("edit_tabwidth"), &options.edit_tabwidth, 0, OPT_EDIT_TABWIDTH_MAX, 1);

	// New appearance settings load
	if (!MyGetstringfromini(hInst, _T("font_name"), options.font_name, ARRAYSIZE(options.font_name)))
	{
	...
	MyGetintfromini(hInst, _T("font_charset"), &val, 0, 255, DEFAULT_CHARSET);
	options.font_charset = (BYTE)val;

	MyGetintfromini(hInst, _T("bg_use_custom"), &options.bg_use_custom, 0, 1, 0);

	MyGetColorfromini(hInst, _T("bg_color"), &val, RGB(255, 255, 255));
	options.bg_color = (COLORREF)val;
...
BOOL OpenHelp(HWND hWnd, HINSTANCE hInst)
{
	...
	if(!BuildPathNextToModule(hInst, _T("multiasm.chm"), szFilePath))
		return FALSE;
