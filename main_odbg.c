...
extc void _export cdecl ODBG_Pluginaction(int origin, int action, void *item)
{
	...
	case PM_MAIN:
		...
		case 1:
			// Menu item "Options"
			ShowOptionsDlg(hwollymain, hDllInst);
			break;
...
extc int _export cdecl ODBG_Pluginshortcut(int origin, int ctrl, int alt, int shift, int key, void *item)
{
	...
	if(key == 'M' && ctrl && !alt)
	{
		// Force double check of physical modifier keys using GetAsyncKeyState
		// Windows occasionally loses WM_KEYUP message during drag/selection focus switches,
		// causing Shift or Ctrl keys to stay 'stuck' (stuck modifier key).
		// We override the logical 'shift' flag with the actual physical key state to prevent selection load confusion.
		BOOL physical_shift = (GetAsyncKeyState(VK_SHIFT) & 0x8000) != 0;
		if (!physical_shift) {
			shift = 0;
		}
		...
		if(!shift)
		{
			// Release all stuck keyboard modifier states to protect the focused dialog
		...
		else if(shift && origin == PM_DISASM)
		{
			pd = (t_dump *)item;
