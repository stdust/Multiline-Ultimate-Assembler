...
void NewTab(HWND hTabCtrlWnd, HWND hAsmEditWnd, TCHAR *pTabLabel)
{
	...
	SendMessage(hAsmEditWnd, SCI_SETTEXT, 0, (LPARAM)"");
...
void OnTabChanging(HWND hTabCtrlWnd, HWND hAsmEditWnd)
{
	...
	tci.extra.selection_start = (int)SendMessage(hAsmEditWnd, SCI_GETSELECTIONSTART, 0, 0);
	tci.extra.selection_end = (int)SendMessage(hAsmEditWnd, SCI_GETSELECTIONEND, 0, 0);
	tci.extra.first_visible_line = (int)SendMessage(hAsmEditWnd, SCI_GETFIRSTVISIBLELINE, 0, 0);
...
void OnTabChanged(HWND hTabCtrlWnd, HWND hAsmEditWnd)
{
	...
	SendMessage(hAsmEditWnd, SCI_SETSEL, tci.extra.selection_start, tci.extra.selection_end);
	SendMessage(hAsmEditWnd, SCI_LINESCROLL, 0, tci.extra.first_visible_line);
	SendMessage(hAsmEditWnd, SCI_SCROLLCARET, 0, 0);
...
void OnTabFileUpdated(HWND hTabCtrlWnd, HWND hAsmEditWnd)
{
	...
	sel_start = (int)SendMessage(hAsmEditWnd, SCI_GETSELECTIONSTART, 0, 0);
	sel_end = (int)SendMessage(hAsmEditWnd, SCI_GETSELECTIONEND, 0, 0);
	first_visible_line = (int)SendMessage(hAsmEditWnd, SCI_GETFIRSTVISIBLELINE, 0, 0);

	LoadFileOfTab(hTabCtrlWnd, hAsmEditWnd);

	SendMessage(hAsmEditWnd, SCI_SETSEL, sel_start, sel_end);
	SendMessage(hAsmEditWnd, SCI_LINESCROLL, 0, first_visible_line);
	SendMessage(hAsmEditWnd, SCI_SCROLLCARET, 0, 0);
}
...
BOOL LoadFileOfTab(HWND hTabCtrlWnd, HWND hAsmEditWnd)
{
	...
	if (dwFileSize == INVALID_FILE_SIZE)
	{
		CloseHandle(hFile);
		ZeroMemory(&ftCurrentTabLastWriteTime, sizeof(FILETIME));
		PostMessage(hPostErrorWnd, uPostErrorMsg, MB_ICONEXCLAMATION, (LPARAM)_T("Could not read file size"));
		return FALSE;
	}

	lpBuffer = (char *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, dwFileSize + 1);
	if (!lpBuffer)
	{
		CloseHandle(hFile);
		ZeroMemory(&ftCurrentTabLastWriteTime, sizeof(FILETIME));
		PostMessage(hPostErrorWnd, uPostErrorMsg, MB_ICONEXCLAMATION, (LPARAM)_T("Memory allocation failed"));
		return FALSE;
	}

	if (ReadFile(hFile, lpBuffer, dwFileSize, &dwReadBytes, NULL))
	{
		lpBuffer[dwReadBytes] = '\0';
		SendMessage(hAsmEditWnd, SCI_SETTEXT, 0, (LPARAM)lpBuffer);
		bRet = TRUE;
	}
	else
	{
		PostMessage(hPostErrorWnd, uPostErrorMsg, MB_ICONEXCLAMATION, (LPARAM)_T("Could not read file data"));
	}
	...
	// Scintilla clear modify point
	SendMessage(hAsmEditWnd, SCI_SETSAVEPOINT, 0, 0);
...
BOOL SaveFileOfTab(HWND hTabCtrlWnd, HWND hAsmEditWnd)
{
	...
	if(!SendMessage(hAsmEditWnd, SCI_GETMODIFY, 0, 0))
		return TRUE;
	...
	nTextSize = (int)SendMessage(hAsmEditWnd, SCI_GETLENGTH, 0, 0);
	lpBuffer = (char *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, nTextSize + 1);
	if (lpBuffer)
	{
		SendMessage(hAsmEditWnd, SCI_GETTEXT, nTextSize + 1, (LPARAM)lpBuffer);
		if (WriteFile(hFile, lpBuffer, nTextSize, &dwWrittenBytes, NULL))
		{
			bRet = TRUE;
		}
		HeapFree(GetProcessHeap(), 0, lpBuffer);
	...
	{
		PostMessage(hPostErrorWnd, uPostErrorMsg, MB_ICONEXCLAMATION, (LPARAM)_T("Failed to write to file"));
	}
	...
	if(GetOpenFileName(&ofn))
	{
		...
		do
		{
			...
			if(hFile != INVALID_HANDLE_VALUE)
			{
				...
				{
					char *lpBuf = (char *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, dwSize + 1);
					if (lpBuf)
					{
						DWORD dwRead;
						...
						{
							lpBuf[dwRead] = '\0';
							SendMessage(hAsmEditWnd, SCI_SETTEXT, 0, (LPARAM)lpBuf);
						}
...
BOOL SaveFileToLibrary(HWND hTabCtrlWnd, HWND hAsmEditWnd, HWND hWnd, HINSTANCE hInst)
{
	...
	int nTextSize = (int)SendMessage(hAsmEditWnd, SCI_GETLENGTH, 0, 0);
	char *lpBuffer = (char *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, nTextSize + 1);
	if (lpBuffer)
	{
		SendMessage(hAsmEditWnd, SCI_GETTEXT, nTextSize + 1, (LPARAM)lpBuffer);
		DWORD dwWritten;
		if (WriteFile(hFile, lpBuffer, nTextSize, &dwWritten, NULL))
		{
			bRet = TRUE;
		}
		HeapFree(GetProcessHeap(), 0, lpBuffer);
